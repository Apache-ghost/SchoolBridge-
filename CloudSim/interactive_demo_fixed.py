"""
Interactive Distributed System Demonstration (Fixed Version)
...
def interactive_distributed_demo():
    ...
    # Show P2P communication setup safely
    print("\n📡 P2P Communication Layer:")
    print(f"  • System ID: {sim.simulation_id}")
    print(f"  • Active Regions: {len(sim.multi_region_manager.regions)}")
    print(f"  • School Nodes: {len(sim.school_nodes)}")
    ...
    for db_id, db_node in sim.database_nodes.items():
        print(f"\n📀 Database Node: {db_id}")
        print(f"  📍 Region: {db_node.region}")
        print(f"  💿 Storage: {db_node.storage_used_gb:.1f}GB / {db_node.storage_capacity_gb}GB")
        print(f"  🔄 Replicas: {len(db_node.replicas)} other nodes")
        print(f"  ⏰ Last Sync: {time.ctime(db_node.last_sync_time)}")
    ...
    if sim.school_nodes:
        ...
        # Simulate failure
        sim.fault_tolerance.simulate_failure("school_node_failure", failing_node.node_id)
